    ...
    <div class="site-container">
      ...
      <div ref="rail" class="pricing-rail" tabindex="0" aria-label="Planos Elínea">
        <article v-for="plan in props.plans" :key="plan.id" class="plan-card"
          ...
          <div class="plan-price" v-if="plan.monthly_amount !== null"><small>R$</small><strong>{{
              money(amount(plan)).replace('R$', '').trim()
            }}</strong><span>/{{ cycle === 'annual' ? 'ano' : 'mês' }}</span></div>
          ...
          <MarketingButton class="plan-action" :variant="plan.featured ? 'primary' : 'outline'" type="button"
            ...
            {{ plan.monthly_amount === null ? 'Falar com especialista' : 'Escolher plano' }}
            <template #icon>
              <ArrowRight :size="16"/>
            </template>
          </MarketingButton>
